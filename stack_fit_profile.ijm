...
macro "stack_fit_profile" {
    ...
    if (imgname == "Stack" || nSlices > 1) {    	
    	...
    	for (i = 1; i <= nSlices; i++) {
    		setSlice(i);
    		d = runMacro("single_fit_profile", args);
  	 		selectWindow("Fit");
  	 		w = getWidth;
  	 		h = getHeight;
  	 		run("Copy");
    		close();
        	if (profile_stack==0) {
            	newImage("Fit Plots", "8-bit", w, h, 1);
            	profile_stack = getImageID;
        	} 
        	else {
	...
	waitForUser("Information", "Profile Fit Completed");
}
